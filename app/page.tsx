...
const menuItems = [
  ...
  { label: 'ЦДУМТМ', icon: BookOpenText }, { label: 'Статьи', icon: BookOpenText },
  { label: 'Медиа', icon: CirclePlay }, { label: 'Контакты', icon: Phone },
];
const sections = [
  { title: 'О нас', icon: Users, links: ['Деятельность ЦДУМТМ', 'Доклады и выступления'] },
  { title: 'Статьи', icon: BookOpenText, links: [
    'Столпы Ислама', 'Пятничные проповеди', 'Основы вероучения', 'Вопросы и ответы',
    'Актуальные темы', 'Библиотека', 'Столпы веры', 'Секты', 'Опровержения',
    'Культура и нравственность', 'Женщина в Исламе', 'Семья в Исламе', 'Суфизм',
    'Вопросы веры', 'Путешествие по странам', 'Восточная медицина', 'История Ислама',
    'Благословенная пятница', 'Неверные мнения о Рамадане', 'Ду\'а и Зикры', 'Тафсир Къур\'ана',
  ] },
  { title: 'Медиа', icon: CirclePlay, links: ['Къур\'ан', 'Фотогалерея', 'Видеогалерея'] },
];

function Brand({ compact = false }: { compact?: boolean }) {
  return <div className="brand" aria-label="ЦДУМТМ">
    ...
    <div className="brand-copy"><strong>ЦДУМТМ</strong>{!compact && <span>Центральное духовное управление<br />мусульман Таврического муфтията</span>}</div>
  </div>;
}
...
export default function Home() {
  ...
  return <div className="site-shell">
    ...
    <main className="empty-canvas" aria-label="Место для будущих новостей" />
